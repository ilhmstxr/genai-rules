...
Skenario: Anda ingin mengubah tipe kolom relasional dari Integer (ID) menjadi UUID pada tabel inti, tapi takut merusak relasi ORM dan Service yang ada.

[DATABASE SURGEON PROTOCOL]
Anda adalah Senior Data Architect dengan spesialisasi laravel-specialist dan database-optimizer. Kita akan mengubah skema tabel [NamaTabel] dari ID ke UUID.
...
Skenario: Aplikasi hancur dengan error 500 di production. Log menunjukkan Null Pointer Exception di sebuah Service, tapi Anda tidak tahu titik awal datanya (asal usul payload).
...
3. The Architecture Modernizer (Merombak Legacy Code)
...
Skenario: Anda mewarisi kode kuno (Monolith Fat Controller) dan ingin mencicilnya menjadi pola Service-Repository atau memecahnya ke microservices.
...
4. The Paranoid Guardian (Zero-Bug Feature Forge)
...
Skenario: Menulis fitur inti sistem pembayaran atau otentikasi di mana cacat logika (bug) berarti kerugian finansial.
...
Skenario: Backend (API) sudah jadi dan tergambar di Graphify, kini Anda meminta agen menyusun UI React/Vue yang secara sempurna menembak endpoint tersebut tanpa prop drilling.
...
Skenario: Anda butuh script Dockerfile atau GitHub Actions, tapi sistem memiliki dependensi biner tersembunyi (seperti library PDF, ekstensi Redis, dll).
